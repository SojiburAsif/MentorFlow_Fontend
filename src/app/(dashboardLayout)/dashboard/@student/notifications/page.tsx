        ...
        {notifications.length === 0 ? (
          ...
          <div className="space-y-3">
            {notifications.map((n: any) => (
              <div key={n.id} className={`bg-[#0d0d1a] border rounded-2xl px-6 py-4 flex items-start gap-4 transition-all ${n.isRead ? "border-sky-900/10 opacity-60" : "border-sky-500/20"}`}>
                <div className="mt-0.5">
                  {n.isRead ? <CheckCircle2 size={16} className="text-slate-600" /> : <Info size={16} className="text-sky-400" />}
                </div>
                ...
                {!n.isRead && <div className="h-2 w-2 rounded-full bg-sky-500 mt-1.5 shrink-0" />}
              </div>
            ))}
          </div>
        )}
