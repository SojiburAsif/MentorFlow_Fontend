/* eslint-disable @typescript-eslint/no-explicit-any */
import AdminUsersTableClient from "@/components/module/admin/AdminUsersTableClient";
import { getAllUsers } from "@/services/user.service";
import { MonitorDot } from "lucide-react";

export default async function AdminTutorsPage() {
  const result = await getAllUsers();
  const all: any[] = result.success ? (Array.isArray(result.data) ? result.data : []) : [];
  const tutors = all.filter((u) => String(u?.role ?? "").toUpperCase() === "TUTOR");

  return (
    <div className="min-h-screen bg-white dark:bg-black text-slate-900 dark:text-blue-400">
      <div className="border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-black px-8 py-5">
        <div className="flex items-center gap-3">
          <div className="p-2 bg-blue-100 dark:bg-blue-600/20 rounded-lg">
            <MonitorDot size={18} className="text-blue-600 dark:text-blue-400" />
          </div>
          <div>
            <h1 className="text-lg font-bold text-blue-600 dark:text-blue-400">Tutor Profiles</h1>
            <p className="text-xs text-slate-600 dark:text-slate-400">{tutors.length} tutor(s)</p>
          </div>
        </div>
      </div>

      <div className="px-8 py-8">
        <AdminUsersTableClient users={tutors} title="Tutors" />
      </div>
    </div>
  );
}
