    ...
    <div className="min-h-screen bg-white dark:bg-black text-slate-900 dark:text-white">
      <div className="border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-black px-8 py-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold text-slate-900 dark:text-white">All Video Calls</h1>
            <p className="text-xs text-slate-600 dark:text-slate-400">{rows.length} call booking(s)</p>
          </div>
        </div>
      </div>
        ...
        <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-black overflow-hidden shadow-sm dark:shadow-md">
          <div className="overflow-auto">
            <table className="min-w-[980px] w-full text-sm">
              ...
              <tbody>
                {rows.map((b) => (
                  <tr key={b.id} className="border-t border-slate-200 dark:border-blue-900/10 hover:bg-slate-100 dark:hover:bg-blue-600/5 transition-colors">
                    <td className="px-4 py-3 font-bold text-blue-600 dark:text-blue-300">{String(b.videoCallId ?? "—")}</td>
                    <td className="px-4 py-3 text-slate-700 dark:text-slate-300">{String(b.status ?? "—")}</td>
                    <td className="px-4 py-3 text-slate-700 dark:text-slate-300">{String(b.paymentStatus ?? "—")}</td>
                    <td className="px-4 py-3">
                      <p className="text-slate-900 dark:text-slate-200 font-semibold">{b.student?.name ?? b.studentId ?? "—"}</p>
                      <p className="text-xs text-slate-600 dark:text-slate-500">{b.student?.email ?? ""}</p>
                    </td>
                    <td className="px-4 py-3">
                      <p className="text-slate-900 dark:text-slate-200 font-semibold">{b.tutor?.name ?? b.tutorId ?? "—"}</p>
                      <p className="text-xs text-slate-600 dark:text-slate-500">{b.tutor?.email ?? ""}</p>
                    </td>
                    <td className="px-4 py-3 text-slate-700 dark:text-slate-300">{fmt(b.dateTime)}</td>
                    <td className="px-4 py-3 text-slate-700 dark:text-slate-300">{b.transactionId ?? b.id}</td>
                  </tr>
                ))}
                {rows.length === 0 && (
                  <tr>
